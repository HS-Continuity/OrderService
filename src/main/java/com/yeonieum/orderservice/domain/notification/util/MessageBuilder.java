...
public class MessageBuilder {
    ...
    public static String createRegularOrderMessage(RegularDeliveryNotificationMessage regularDeliveryNotificationMessage) {

        // 정기배송 신청 완료 시 메시지
        return "[연이음] 정기배송 신청완료 안내\n" +
                "\n" +
                "안녕하세요," + regularDeliveryNotificationMessage.getMemberName() + "님!\n" +
                ...
                "\n" +
                "▶정기배송지 : " + regularDeliveryNotificationMessage.getAddress() + "\n" +
                "▶정기배송상품 :" +  regularDeliveryNotificationMessage.getProductName() + " 외 "+ regularDeliveryNotificationMessage.getProductCount() + "건\n" +
                "▶다음 정기배송일 : " + regularDeliveryNotificationMessage.getNextDeliveryDate().toString() + "일";
    }

    public static String cancelRegularOrderMessage(RegularDeliveryNotificationMessage regularDeliveryNotificationMessage) {
        // 정기배송 신청 취소 시 메시지
    ...
    public static String postponeRegularOrderMessage(RegularDeliveryNotificationMessage regularDeliveryNotificationMessage) {
        // 정기배송 일시 정지 시 메시지
        return "[연이음] 정기배송 회차 건너뛰기 안내\n" +
                "\n" +
                "안녕하세요," + regularDeliveryNotificationMessage.getMemberName() + "님!\n" +
                "고객님의 정기배송 서비스가 일시 정지되었습니다. \n" +
                "정기배송 서비스 " + regularDeliveryNotificationMessage.getCompletedOrderCount() + "회차 정기배송 상품이 배송되지 않습니다.\n" +
                "\n" +
                "▶정기배송상품 :" +  regularDeliveryNotificationMessage.getProductName() + " 외 "+ regularDeliveryNotificationMessage.getProductCount() + "건\n";
