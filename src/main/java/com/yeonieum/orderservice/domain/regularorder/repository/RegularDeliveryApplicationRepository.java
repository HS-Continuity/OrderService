...
public interface RegularDeliveryApplicationRepository extends JpaRepository<RegularDeliveryApplication, Long> {
    @Query("SELECT r FROM RegularDeliveryApplication r WHERE r.memberId = :memberId ORDER BY r.createdAt ASC")
    Page<RegularDeliveryApplication> findByMemberIdOrderByCreatedAtAsc(String memberId, Pageable pageable);
